...
sufficiently large~$n$. Thus every strictly increasing sequence of derivative
orders has zero sets with dense union. The construction uses a random Fock
series with independent coefficients uniformly distributed on the closed unit
disk. A saddle-point estimate supplies a dominant Taylor coefficient.
Combining one-coordinate anti-concentration with the mean value property of
$\log|f^{(n)}|$ yields a summable bound for the probability that $f^{(n)}$ is
zero-free on a fixed disk. Every sample satisfies
$|f(z)|\leq\sqrt2\exp(|z|^2)$. The resulting function is a counterexample to
Boas and Reddy's printed assertion that every entire function of order at most
two and finite type admits an arbitrarily large fixed disk zero-free for
infinitely many successive derivatives.
\end{abstract}
...
status may miss relevant literature \cite{Bloom906}. We therefore give a
